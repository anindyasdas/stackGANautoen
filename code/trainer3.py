...
from model1 import encoder1, G_NET, D_NET64, D_NET128, D_NET256, D_NET512, D_NET1024, INCEPTION_V3
...
class condGANTrainer(object):
    ...
    def train_Dnet(self, idx, count):
        ...
        # update parameters
        optD.step()
        #self.nwSwitch = not(self.nwSwitch)
        # log
        if flag == 0:
    ...
    def train_Gnet(self, count):
        ...
        for i in range(self.num_Ds):
            ...
            errM = criterion1(self.fake_imgs[i], self.real_imgs[i])*5000.0
            ...
            errG_total = errG_total + errG*4.0
        ...
        if cfg.TRAIN.COEFF.COLOR_LOSS > 0:
            ...
            if self.num_Ds > 2:
                mu1, covariance1 = compute_mean_covariance(self.real_imgs[0])
                mu2, covariance2 = \
                    compute_mean_covariance(self.fake_imgs[-3])
                like_mu0 = cfg.TRAIN.COEFF.COLOR_LOSS * nn.MSELoss()(mu1, mu2)
                ...
                like_cov1 = cfg.TRAIN.COEFF.COLOR_LOSS * 5 * \
                    nn.MSELoss()(covariance1, covariance2)
                #print('mu:', mu1, covariance1)
                #print('coeff:', cfg.TRAIN.COEFF.COLOR_LOSS)
                #print(like_mu1 , like_cov1 , like_mu0 ,like_cov0)
        ...
        if True:
            ...
            errG_total.backward()
            errG_total = errG_total- errM_total
        #else:
    ...
    def train(self):
        ...
        #self.criterion1 = nn.MSELoss(size_average=False)
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            for step, data in enumerate(self.data_loader, 0):
                ...
                self.txt_embedding = self.enc(self.real_imgs[0])
                ...
                if count % 100 == 0:
                    ...
                    summary_M = summary.scalar('M_loss', errM_total.item())
            ...
            end_t = time.time()
            print('''[%d/%d][%d]
                         Loss_D: %.2f Loss_G: %.2f Loss_MSE: %.2f Loss_KL: %.2f Time: %.2fs
                      '''  # D(real): %.4f D(wrong):%.4f  D(fake) %.4f
                  % (epoch, self.max_epoch, self.num_batches,
                     errD_total.item(), errG_total.item(), errM_total.item(),
