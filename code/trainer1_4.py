...
from model1 import  encoder_resnet, G_NET1, D_NET64, D_NET128, D_NET256, D_NET512, D_NET1024, INCEPTION_V3
...
class condGANTrainer(object):
    def __init__(self, output_dir, data_loader, imsize):
        ...
        self.norm1 = transforms.Compose([transforms.Normalize(mean=[-1, -1, -1], std=[2, 2, 2])])#to covert -1 to 1 into 0 to 1 range
        self.norm2 = transforms.Compose([transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
    ...
    def train_Gnet(self, count):
        ...
        for i in range(self.num_Ds):
            ...
            errM_total = errM_total + errM*25.0
            if flag == 0:
        ...
        #emb_MSE_loss = criterion1(self.txt_embedding_cyl, self.txt_embedd.detach()) 
        #emb_m = criterion1(self.mu_cyl, self.m.detach())
        #emb_v = criterion1(self.logvar_cyl, self.v.detach())
        #emb_loss = (emb_MSE_loss + emb_m + emb_v*5)*25.0
        err = errG_total + kl_loss + errM_total #
        #emb_MSE_loss.backward() #calculate the gradient
        #self.enc.zero_grad() #don't want to change gradient on ecoder
        err.backward() # calculate grad for err
        torch.nn.utils.clip_grad_norm_(self.netG.parameters(), 5.00)
        torch.nn.utils.clip_grad_norm_(self.enc.parameters(), 5.00)
        self.optimizerG.step()
        return kl_loss, errG_total, errM_total#
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            for step, data in enumerate(self.data_loader, 0):
                ...
                #print('range of ureal img :', torch.max(self.ureal_imgs[0]), torch.min(self.ureal_imgs[0]))
                #print('self.txt_embedding', self.txt_embedding.shape)
                ...
                #print('self.fake_imgs', self.fake_imgs[0].shape)
                    
                #################Encoding from Generated image##########
                #fake_imgs = self.netG(noise, self.txt_embedding.detach())
                #print('fake_imgs', fake_imgs[0].shape)
                #print('range of fake img:', torch.max(fake_imgs[0]), torch.min(fake_imgs[0]))
                #self.txt_embedding_cyl, self.mu_cyl, self.logvar_cyl = self.enc(fake_imgs[0])
                #print('self.txt_embedding_cyl', self.txt_embedding_cyl.shape)
                #self.txt_embedd, self.m, self.v = self.enc(self.real_imgs[0])
                #print('range of real img :', torch.max(self.real_imgs[0]), torch.min(self.real_imgs[0]))
                #print('self.txt_embedd', self.txt_embedd.shape)
            ...
            end_t = time.time()
            print('''[%d/%d][%d]
                         Loss_D: %.2f Loss_G: %.2f Loss_KL: %.2f Loss_MSE: %.2f  Time: %.2fs
                      '''  # D(real): %.4f D(wrong):%.4f  D(fake) %.4f
                  % (epoch, self.max_epoch, self.num_batches,
                     errD_total.item(), errG_total.item(), 
                     kl_loss.item(), errM_total.item(), end_t - start_t))
